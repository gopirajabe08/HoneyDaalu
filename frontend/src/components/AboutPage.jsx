    ...
    <>
      ...
      <div className="bg-dark-700 rounded-xl border border-dark-500 p-5">
        ...
        <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-xs text-gray-400">
          ...
          <div className="flex items-start gap-2"><span className="text-orange-400 mt-0.5">&#9679;</span><span>Lot sizes: NIFTY = 75, BANKNIFTY = 30</span></div>
...
function AppPagesTab() {
  const pages = [
    ...
    { name: 'Intraday Trading', desc: 'Live/Paper toggle on a single page. On-demand scan mode: initial scan at 12 PM, re-scans when slot opens. Max 4 positions (live) / 10 (paper). Price filter: Rs 50 - Rs 5,000. Daily Strategy Performance section shows per-day strategy breakdown with timeframe.', color: 'text-blue-400', bg: 'bg-blue-500/10 border-blue-500/20' },
    ...
    { name: 'Options Intraday (Live/Paper)', desc: 'NIFTY/BANKNIFTY options spread trading. Auto-detects market regime (VIX, PCR, Nifty trend) and selects highest-probability strategy. 6 spread strategies: Bull Put, Bear Call, Iron Condor (credit, ~65-70% win rate), Bull Call, Bear Put (debit), Long Straddle (high vol). Orders 10 AM - 2 PM, square-off 3 PM.', color: 'text-violet-400', bg: 'bg-violet-500/10 border-violet-500/20' },
...
function RulesTab() {
  const rules = [
    ...
    { label: 'Risk Per Trade', value: '2% of capital', icon: Shield },
