...
export default function OptionsIntradayTrade({ mode, capital, setCapital }) {
  ...
  const accentGrad = isLive ? 'from-emerald-500 to-cyan-500' : 'from-blue-500 to-cyan-500'
  const accentBorder = isLive ? 'border-emerald-500/20' : 'border-blue-500/20'
  const accentBg = isLive ? 'from-emerald-500/5 to-cyan-500/5' : 'from-blue-500/5 to-cyan-500/5'
  ...
  const pollStatus = useCallback(async () => {
    try {
      ...
      setStatus(data)
      // For live mode, fetch TradeJini positions as source of truth
      if (isLive) {
        try {
          ...
          setBrokerPositions(optionsOnly)
        } catch {}
      }
    } catch {}
  }, [isLive])
  ...
  // Poll status every 10s — always for live (TradeJini data), only when running for paper
  useEffect(() => {
    if (running || isLive) {
    ...
    <div className="space-y-5">
      ...
      <div className="flex gap-6">
        ...
        <div className="flex-1 min-w-0 space-y-4">
          ...
          {isLive && brokerPositions.length > 0 && (
            <div className="bg-dark-700 rounded-2xl border border-emerald-500/20 p-5">
              <h3 className="text-sm font-semibold text-white mb-3 flex items-center gap-2">
                ...
                TradeJini Options Positions (Source of Truth)
          ...
          {/* Trade History -- paper only (live uses TradeJini table above) */}
          {!isLive && (
            <div className="bg-dark-700 rounded-2xl border border-dark-500 p-5">
              <h3 className="text-sm font-semibold text-white mb-3 flex items-center gap-2">
                        ...
                        return (
                          <button key={sym} onClick={() => toggleUnderlying(sym)}
                            className={`w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl border transition-all ${
                              isSelected ? `bg-dark-600 ${isLive ? 'border-emerald-500/30' : 'border-blue-500/30'}` : 'bg-dark-800 border-dark-600 hover:border-dark-500'
                            }`}>
                  ...
                  {isLive && (
                    <div className="mt-2 flex items-start gap-2 bg-red-500/5 border border-red-500/20 rounded-lg px-3 py-2">
                      ...
                      <p className="text-[9px] text-red-400 leading-relaxed">
                        Live mode places real options orders on TradeJini. Real money at risk.
                      </p>
                    </div>
                  )}
                </>
